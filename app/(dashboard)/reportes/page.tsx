...
import {
  ...
  Clock, Eye, FileCheck, FileText, FileUp, Gavel, Inbox, Loader2, Search,
  TrendingUp, UserPlus, X, Archive,
} from 'lucide-react'
...
interface TesisReporte {
  id: string; codigo: string; titulo: string; estado: string; fase: string
...
interface Resumen { total: number; proyectosAprobados: number; informesAprobados: number; enProceso: number }
...
export default function ReportesPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => { setBusquedaDebounced(busqueda); setPage(1) }, 400)
  ...
  const loadReportes = useCallback(async () => {
    try {
      ...
      const result = await api.get<{
        data: TesisReporte[]; contadores: Contadores; resumen: Resumen
        facultades: Facultad[]; pagination: { totalPages: number; totalItems: number }
      }>('/api/reportes', {
        params: {
      ...
      setTesis(result.data); setContadores(result.contadores); setResumen(result.resumen)
      setFacultades(result.facultades); setTotalPages(result.pagination.totalPages); setTotalItems(result.pagination.totalItems)
    } catch (error) {
      console.error('Error cargando reportes:', error)
    } finally { setLoading(false); setInitialLoading(false) }
  }, [page, filtroEstado, filtroFacultad, busquedaDebounced])

  useEffect(() => { if (!authLoading && user) loadReportes() }, [authLoading, user, loadReportes])

  const handleFiltroEstado = (estado: string) => { setPage(1); setFiltroEstado(estado) }
  const handleFiltroFacultad = (fId: string) => { setPage(1); setFiltroFacultad(fId) }
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">No tienes permisos para acceder al módulo de Reportes.</p>
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Estado general de las tesis en el sistema</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {[
            { label: 'Total', value: resumen.total, icon: BookOpen, color: 'text-blue-500' },
            { label: 'Proy. Aprob.', value: resumen.proyectosAprobados, icon: CheckCircle, color: 'text-emerald-500' },
            { label: 'Inf. Aprob.', value: resumen.informesAprobados, icon: FileCheck, color: 'text-green-500' },
            { label: 'En Proceso', value: resumen.enProceso, icon: TrendingUp, color: 'text-amber-500' },
          ].map((kpi) => (
            <div key={kpi.label} className="flex items-center gap-2 rounded-xl border px-3 py-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          <button
            ...
            className={cn(
              ...
              filtroEstado === 'TODOS' ? 'bg-primary text-primary-foreground shadow-sm' : 'border bg-card hover:bg-accent text-muted-foreground'
            )}
          >
            Todos <span className={cn('tabular-nums', filtroEstado === 'TODOS' ? 'text-primary-foreground/70' : 'text-muted-foreground/60')}>{resumen.total}</span>
          </button>
          {estadosConDatos.map((estado) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all',
                  isActive ? 'bg-primary text-primary-foreground shadow-sm' : 'border bg-card hover:bg-accent'
                )}
              >
          ...
          {estadosSinDatos.length > 0 && (
            <span className="inline-flex items-center px-2 py-1.5 text-[10px] text-muted-foreground/40">+{estadosSinDatos.length} sin tesis</span>
          )}
        </div>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-3 p-4 border-b bg-muted/20">
          <div className="relative flex-1">
            ...
            <Input placeholder="Buscar por título o tesista..." className="pl-9 bg-background" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
          </div>
          ...
          <Select value={filtroFacultad} onValueChange={handleFiltroFacultad}>
            <SelectTrigger className="w-full sm:w-44 bg-background"><SelectValue placeholder="Facultad" /></SelectTrigger>
            <SelectContent>
        ...
        <div className={cn('relative', loading && 'opacity-50 pointer-events-none')}>
          ...
          {tesis.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {tesis.map((t) => {
                  const ec = ESTADO_CONFIG[t.estado]
                  return (
                    <TableRow key={t.id} className="group cursor-pointer" onClick={() => router.push(`/mesa-partes/${t.id}`)}>
                      <TableCell><span className="font-mono text-[11px] font-medium text-muted-foreground">{t.codigo}</span></TableCell>
                      <TableCell>
                        <div className="min-w-0 max-w-[320px]">
                          <p className="font-medium text-sm leading-snug line-clamp-1 group-hover:text-primary transition-colors">{t.titulo}</p>
                      ...
                      <TableCell className="hidden lg:table-cell">
                        <span className="text-xs text-muted-foreground truncate block max-w-[150px]">{t.asesor?.nombre || 'Sin asesor'}</span>
                      </TableCell>
                      <TableCell className="hidden md:table-cell">
                        {t.facultad && <Badge variant="secondary" className="text-[10px] font-medium">{t.facultad.codigo}</Badge>}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" asChild className="h-8 w-8 p-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                          <Link href={`/mesa-partes/${t.id}`}><Eye className="w-4 h-4" /></Link>
                        </Button>
                      </TableCell>
                    </TableRow>
                  )
        ...
        {totalItems > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t">
            <p className="text-xs text-muted-foreground">
              ...
              {' de '}<span className="font-medium text-foreground">{totalItems}</span> tesis
            </p>
            {totalPages > 1 && (
